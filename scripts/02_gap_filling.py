...
tdfs_15m_gapfilled = []
for i, (df_15m, df_60m, tname) in enumerate(
    zip(tower_dfs_15m, tower_dfs_60m, towers_of_interest)
):
    print(tname)
    ...
    for col_name in df_15m.columns.to_list():
        ...
        mid_gaps = missing_data[
            (missing_data["gap_length_hrs"] >= 3)
            & (missing_data["gap_length_hrs"] <= 6)
        ]
...
def add_buffer(df_15m, col_name, buffer_hours=3):
    ...
    mean_buffer = df_15m.groupby(
        [df_15m.index.month, df_15m.index.day, df_15m.index.hour]
    )[col_name].median()

    start_date = df_15m.index[0] - pd.Timedelta(hours=buffer_hours)
    start_buffer = [
        mean_buffer[(start_date.month, start_date.day, hour)]
        for hour in range(start_date.hour, start_date.hour + buffer_hours)
    ]

    end_date = df_15m.index[-1] + pd.Timedelta(hours=buffer_hours)
    end_buffer = [
        mean_buffer[(end_date.month, end_date.day, hour)]
        for hour in range(end_date.hour - buffer_hours + 1, end_date.hour + 1)
    ]

    buffer_start = pd.Series(
        start_buffer,
        index=pd.date_range(start=start_date, periods=buffer_hours, freq="h"),
    )
    buffer_end = pd.Series(
        end_buffer,
        index=pd.date_range(
            start=df_15m.index[-1] + pd.Timedelta(hours=1),
            periods=buffer_hours,
            freq="h",
        ),
    )
...
for i, (df_15m, tname) in enumerate(zip(tdfs_15m_gapfilled, towers_of_interest)):
    ...
    for col_name in df_15m.columns.to_list():
        ...
        try:
            ...
            smoothed_col = savgol_filter(
                df_with_buffer.values, window_length=11, polyorder=2, mode="nearest"
            )
            smoothed_df = pd.DataFrame(
                smoothed_col, index=df_with_buffer.index, columns=[col_name]
            )

            # PCHIP interpolation
            smoothed_df_pchip = smoothed_df.copy()
            smoothed_df_pchip[col_name] = smoothed_df_pchip[col_name].interpolate(
                method="pchip", limit=12, limit_direction="both"
            )
            smoothed_df_pchip[col_name] = global_clip(
                smoothed_df_pchip[col_name], lower_percentile=1, upper_percentile=99
            )
            smoothed_df_pchip[col_name] = rolling_clip(smoothed_df_pchip[col_name])
            df_15m_col[col_name] = df_15m_col[col_name].fillna(
                smoothed_df_pchip[col_name]
            )
            ...
            smoothed_df_linear[col_name] = smoothed_df_linear[col_name].interpolate(
                method="linear", limit=12, limit_direction="both"
            )
            smoothed_df_linear[col_name] = global_clip(
                smoothed_df_linear[col_name], lower_percentile=1, upper_percentile=99
            )
            smoothed_df_linear[col_name] = rolling_clip(smoothed_df_linear[col_name])
            df_15m_col_linear[col_name] = df_15m_col_linear[col_name].fillna(
                smoothed_df_linear[col_name]
            )
...
for tdf, tdf_gf_linear, tdf_gf_pchip in zip(
    tower_dfs_15m, tdfs_15m_gapfilled_linear, tdfs_15m_gapfilled_pchip
):
    renamed_tdf = rename_columns(
        tdf, label_conversions
    )  # Get renamed DataFrame for tdf
    renamed_tdf_gf_linear = rename_columns(
        tdf_gf_linear, label_conversions
    )  # Get renamed DataFrame for tdf_gf
...
for tdf_15m, gapfilled_tdf, tname in zip(
    renamed_dfs_15m, renamed_dfs_15m_gf_linear, towers_of_interest
):
    ...
    plot_missing_data_with_sparkline(
        tdf_15m, ax_matrix_1, ax_sparkline_1, f"(A) {tname} Original Data"
    )
    ...
    plot_missing_data_with_sparkline(
        gapfilled_tdf, ax_matrix_2, ax_sparkline_2, f"(B) {tname} Gap-filled Data"
    )
...
with open(csv_file_path, mode="w", newline="") as file:
    ...
    for tdf_15m, tdf_15m_gapfilled_pchip, tdf_15m_gapfilled_linear, tower_name in zip(
        renamed_dfs_15m,
        renamed_dfs_15m_gf_pchip,
        renamed_dfs_15m_gf_linear,
        towers_of_interest,
    ):
        ...
        for column in tdf_15m.columns:
            ...
            mask = np.random.choice(
                [True, False], size=tdf_15m[column].shape, p=[0.8, 0.2]
            )
            ...
            train_data[~mask] = (
                np.nan
            )  # Replace test data points with NaN in the training set
            ...
            rmse_pchip = np.sqrt(
                mean_squared_error(test_data_aligned, pchip_filled_aligned)
            )
            rmse_linear = np.sqrt(
                mean_squared_error(test_data_aligned, linear_filled_aligned)
            )
            ...
            writer.writerow(
                [tower_name, variable, height, rmse_pchip_rounded, rmse_linear_rounded]
            )
...
for tdf, tower in zip(tdfs_15m_gapfilled_linear, towers_of_interest):
    ...
    tdf.to_csv(
        f"../data/met_towers_2017-2022_gapfilled-qc/{tower}_2017-2022_gapfilled-qc.csv",
        encoding="utf-8-sig",
    )
...
((tower_dfs_15m[0].fillna(-999)).ne((tdfs_15m_gapfilled_linear[0].fillna(-999)))).head(
    5
)
for df_orig, df_filled, tower in zip(
    tower_dfs_15m, tdfs_15m_gapfilled_linear, towers_of_interest
):
    ...
    bool_df.to_csv(
        f"../data/met_towers_2017-2022_gapfilled-bool/{tower}_2017-2022_gapfilled-bool.csv",
...
og_df = pd.read_csv(
    f"../data/met_towers_2017-2022_original-qc/{example_tower}_2017-2022_original-qc.csv",
    na_values=[-999],
)
...
gf_df = pd.read_csv(
    f"../data/met_towers_2017-2022_gapfilled-qc/{example_tower}_2017-2022_gapfilled-qc.csv",
    na_values=[-999],
)
...
for col in gf_df2.columns:
    ...
    y_label_match = re.match(
        r"^[^()]+(?:\s*\([^)]*\))?", col
    )  # Match variable name with parentheses
    y_label = (
        y_label_match.group(0).strip() if y_label_match else col
    )  # Includes parentheses

    # Extract variable name without parentheses for title
    y_label_no_parentheses = re.sub(
        r"\s*\(.*?\)", "", y_label
    ).strip()  # Removes parentheses and content inside

    # Extract sensor height (e.g., "15-m") if it exists
    height_match = re.search(
        r"at\s([\d\w-]+)", col
    )  # Match "at 15-m" and extract "15-m"
    ...
    if col in og_df2.columns:
        ...
        axes[0].set_title(
            f"(A) Original Tower {tower_letter} data at {height} sensor height"
        )
    else:
        axes[0].text(
            0.5,
            0.5,
            "Column data was not present",
    ...
    axes[1].set_title(
        f"(B) Gap-filled Tower {tower_letter} data at {height} sensor height"
    )
